...
```swift
Web3Modal.selectChain(from: self)
```

Web3Modal will show list of chain for user. After user select a chain, the list of wallet will show for user to choose and connect. After users connect their wallet, the `sessionSettlePublisher` will publish an event. After this event emit, Call `startAuth` to create a challenge message.
